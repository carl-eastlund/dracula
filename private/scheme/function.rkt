...
(define-if-unbound (identity x) x)
...
(define-syntax-if-unbound (thunk stx)
  (syntax-case stx ()
    [(thunk body ...)
     (syntax/loc stx
...
(define-syntax-if-unbound eta*
  (syntax-rules ()
    [(_ f arg ...) (lambda (arg ...) (f arg ...))]
    [(_ f arg ... . rest) (lambda (arg ... . rest) (apply f arg ... rest))]))

(define-syntax-if-unbound eta
  (syntax-rules () [(_ f) (make-eta-expansion (lambda () f))]))
...
(define-syntax-if-unbound (lambda/parameter stx)
  (syntax-case stx ()
    [(_ p-args . body)

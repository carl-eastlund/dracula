...
(define (if/nil q a e)
  (r.if (equal? q 'nil) (e) (a)))

(define (atom x) (r.if (pair? x) 'nil 't))
(define (car x) (r.if (pair? x) (r.car x) '()))
(define (cdr x) (r.if (pair? x) (r.cdr x) '()))
(define (equal x y) (r.if (equal? x y) 't 'nil))
(define (natp x)
  (r.if (integer? x) (r.if (< x 0) 'nil 't) 'nil))
(define (+ x y) (r.+ (num x) (num y)))
(define (< x y)
  (r.if (r.< (num x) (num y)) 't 'nil))
